...
class GitConfigurationTest < Test::Unit::TestCase
  ...
  def test_remote_master_info_with_should_only_use_username_with_ssh_protocol
    config = GitConfiguration.new(:repository_path => 'ssh://git.serpentine.com:1234/tutorial/hello.git')
    config.project = Project.new
    config.username = 'sammy'
    config.password = 'soso'
    assert_equal 'ssh://sammy@git.serpentine.com:1234/tutorial/hello.git', config.remote_master_info.path
    assert_equal 'ssh://sammy@git.serpentine.com:1234/tutorial/hello.git', config.remote_master_info.log_safe_path
  end
  ...
  def test_deletes_any_existing_cache_directory_upon_creation
    ...
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'repository'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'repository', 'foo.txt'))
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'source_browser_cache'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'source_browser_cache', 'bar.txt'))
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_id, 'source_browser_cache'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_id, 'source_browser_cache', 'decoy.txt'))
    GitConfiguration.create!(:repository_path => '/foo/bar')
    assert !File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'repository', 'foo.txt'))
    assert !File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', expected_id, 'source_browser_cache', 'bar.txt'))
    assert File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_id, 'source_browser_cache', 'decoy.txt'))
  end
  ...
  def test_cache_files_are_deleted_upon_destroy
    ...
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'repository'))
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'source_browser_cache'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'repository', 'foo.txt'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'source_browser_cache', 'bar.txt'))
    FileUtils.mkdir_p(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_config_id, 'source_browser_cache'))
    FileUtils.touch(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_config_id, 'source_browser_cache', 'decoy.txt'))
    config.destroy
    assert !File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'repository', 'foo.txt'))
    assert !File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', config_id, 'source_browser_cache', 'bar.txt'))
    assert File.exist?(File.join(MINGLE_DATA_DIR, 'plugin_data', 'mingle_git_plugin', another_config_id, 'source_browser_cache', 'decoy.txt'))
  end
